...
func (r *PostgresRepository) List(ctx context.Context, tenantID, siteID string) ([]Zone, error) {
	...
	if siteID != "" {
		...
		query, args = zoneSelect+" WHERE site_id = $1::uuid ORDER BY id LIMIT 500", []any{siteID}
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("commit zone list: %w", err)
	}
	return result, nil
}
...
func (r *PostgresRepository) Get(ctx context.Context, tenantID, zoneID string) (Zone, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	zone, err := scanZone(tx.QueryRow(ctx, zoneSelect+" WHERE id = $1::uuid", zoneID))
	...
	if err := tx.Commit(ctx); err != nil {
		return Zone{}, fmt.Errorf("commit zone read: %w", err)
	}
	return zone, nil
}
...
func (r *PostgresRepository) Create(ctx context.Context, command CreateCommand) (CreateResult, error) {
	...
	if _, err := tx.Exec(ctx, `DELETE FROM platform.idempotency_keys WHERE tenant_id = $1::uuid AND idempotency_key = $2 AND expires_at <= clock_timestamp()`, command.TenantID, command.IdempotencyKey); err != nil {
		return CreateResult{}, fmt.Errorf("expire zone idempotency key: %w", err)
	}
	...
	if err == nil {
		...
		var zone Zone
		if err := json.Unmarshal(storedResponse, &zone); err != nil {
			return CreateResult{}, fmt.Errorf("decode idempotent zone response: %w", err)
		}
		if err := tx.Commit(ctx); err != nil {
			return CreateResult{}, fmt.Errorf("commit zone replay: %w", err)
		}
		return CreateResult{Zone: zone, Replayed: true}, nil
	}
	if err != pgx.ErrNoRows {
		return CreateResult{}, fmt.Errorf("read zone idempotency key: %w", err)
	}
	var siteExists bool
	if err := tx.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM config.sites WHERE id = $1::uuid AND tenant_id = $2::uuid AND status <> 'retired')`, command.SiteID, command.TenantID).Scan(&siteExists); err != nil {
		return CreateResult{}, fmt.Errorf("validate zone site: %w", err)
	}
	if !siteExists {
	...
	if err != nil {
		return CreateResult{}, fmt.Errorf("encode zone response: %w", err)
	}
	if _, err := tx.Exec(ctx, `INSERT INTO platform.idempotency_keys (tenant_id, idempotency_key, request_hash, response_status, resource_type, resource_id, response_body) VALUES ($1::uuid, $2, $3, 201, 'zone', $4::uuid, $5::jsonb)`, command.TenantID, command.IdempotencyKey, hash, zone.ID, response); err != nil {
		return CreateResult{}, fmt.Errorf("store zone idempotency key: %w", err)
	}
	if _, err := audit.Append(ctx, tx, audit.Event{TenantID: command.TenantID, ActorID: command.ActorID, Action: "zone.created", ResourceType: "zone", ResourceID: zone.ID, RequestID: command.RequestID, AfterState: zone, OccurredAt: zone.CreatedAt}); err != nil {
		return CreateResult{}, err
	}
	if err := tx.Commit(ctx); err != nil {
		return CreateResult{}, fmt.Errorf("commit zone creation: %w", err)
	}
	return CreateResult{Zone: zone}, nil
}
...
func (r *PostgresRepository) Update(ctx context.Context, command UpdateCommand) (Zone, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	current, err := scanZone(tx.QueryRow(ctx, zoneSelect+" WHERE id = $1::uuid FOR UPDATE", command.ZoneID))
	if errors.Is(err, pgx.ErrNoRows) {
		return Zone{}, ErrNotFound
	}
	if err != nil {
	...
	if err := tx.Commit(ctx); err != nil {
		return Zone{}, fmt.Errorf("commit zone update: %w", err)
	}
	return next, nil
}
...
func (r *PostgresRepository) begin(ctx context.Context, tenantID string, mode pgx.TxAccessMode) (pgx.Tx, error) {
	if r == nil || r.pool == nil {
		return nil, errors.New("postgres zone repository is unavailable")
	}
	...
	if _, err := tx.Exec(ctx, "SELECT set_config('app.tenant_id', $1, true)", tenantID); err != nil {
		_ = tx.Rollback(ctx)
		return nil, fmt.Errorf("set zone tenant context: %w", err)
	}
	return tx, nil
}
