...
func TestZoneRoutesEnforceScopeGeometryAndVersions(t *testing.T) {
	...
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), "Loading bay perimeter") { t.Fatalf("list zones failed: %d %s", listed.Code, listed.Body.String()) }
	var id string
	for _, candidate := range []string{zoneID} { id = candidate }
	...
	stored, err := repository.List(t.Context(), cameraTenant, cameraSite); if err != nil || len(stored) != 1 { t.Fatalf("read stored zone: %v", err) }; id = stored[0].ID
	updated := cameraRequest(handler, http.MethodPatch, "/v1/zones/"+id, `{"config_version":1,"enabled":false}`, nil)
	if updated.Code != http.StatusOK || !strings.Contains(updated.Body.String(), `"config_version":2`) || !strings.Contains(updated.Body.String(), `"enabled":false`) { t.Fatalf("update zone failed: %d %s", updated.Code, updated.Body.String()) }
	if stale := cameraRequest(handler, http.MethodPatch, "/v1/zones/"+id, `{"config_version":1,"enabled":true}`, nil); stale.Code != http.StatusConflict { t.Fatalf("stale zone update: %d %s", stale.Code, stale.Body.String()) }
}
...
func TestZoneRoutesHideCrossSiteAndRejectInvalidGeometry(t *testing.T) {
	...
	handler := zoneHandler(principal, repository)
	if response := cameraRequest(handler, http.MethodGet, "/v1/zones/"+zoneID, "", nil); response.Code != http.StatusNotFound { t.Fatalf("cross-site zone must be hidden: %d", response.Code) }
	...
	if response := cameraRequest(writer, http.MethodPost, "/v1/zones", invalid, map[string]string{idempotencyHeader: "invalid-zone"}); response.Code != http.StatusUnprocessableEntity { t.Fatalf("invalid geometry: %d %s", response.Code, response.Body.String()) }
}
