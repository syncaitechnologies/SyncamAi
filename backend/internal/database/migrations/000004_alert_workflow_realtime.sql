...
CREATE TABLE IF NOT EXISTS realtime.site_sequences (
    tenant_id uuid NOT NULL REFERENCES identity.tenants(id),
...
CREATE INDEX IF NOT EXISTS realtime_messages_replay_idx
    ON realtime.messages (tenant_id, site_id, sequence, expires_at);
CREATE INDEX IF NOT EXISTS realtime_messages_expiry_idx
    ON realtime.messages (expires_at);

ALTER TABLE alerts.alert_actions ENABLE ROW LEVEL SECURITY;
ALTER TABLE alerts.alert_actions FORCE ROW LEVEL SECURITY;
ALTER TABLE realtime.site_sequences ENABLE ROW LEVEL SECURITY;
ALTER TABLE realtime.site_sequences FORCE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS tenant_isolation ON realtime.site_sequences;
CREATE POLICY tenant_isolation ON realtime.site_sequences
    USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON realtime.messages;
CREATE POLICY tenant_isolation ON realtime.messages
    USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid);
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'syncam_app') THEN
        EXECUTE 'GRANT USAGE ON SCHEMA realtime TO syncam_app';
        EXECUTE 'GRANT SELECT, INSERT ON alerts.alert_actions TO syncam_app';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE ON realtime.site_sequences TO syncam_app';
        EXECUTE 'GRANT SELECT, INSERT, DELETE ON realtime.messages TO syncam_app';
