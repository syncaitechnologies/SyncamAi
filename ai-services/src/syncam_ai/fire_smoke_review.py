"""Synthetic-safe temporal confirmation for future fire/smoke metadata.

No detector, image, crop, sensor command, or autonomous action enters this
module. Vision events remain pending human review and complement rather than
replace certified fire-detection and alarm systems.
"""
...
FIRE_SMOKE_CLASSES: Final = frozenset({"fire", "smoke"})
FIRE_CONFIRMATION_FRAMES: Final = 3
SMOKE_CONFIRMATION_FRAMES: Final = 5
...
MAX_SMOKE_RISE_PER_FRAME: Final = 0.25
_MAX_SEQUENCE: Final = (1 << 63) - 1
_MODEL_VERSION = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]{0,127}")
_EVENT_NAMESPACE: Final = UUID("81228054-2a48-44d3-9921-9da718480304")


@dataclass(frozen=True, slots=True)
class FireSmokeCandidate:
...
class FireSmokeCandidateFrame:
    ...
    candidates: tuple[FireSmokeCandidate, ...]


@dataclass(frozen=True, slots=True)
class FireSmokeReviewMetrics:
...
class FireSmokeReviewConfirmation:
    """Confirm fire or rising-smoke metadata before review-only emission."""
    ...
    def __init__(
        ...
        *,
        ...
        minimum_smoke_rise: float = 0.001,
        ...
        if (
            isinstance(minimum_smoke_rise, bool)
            or not isinstance(minimum_smoke_rise, (int, float))
            or not isfinite(float(minimum_smoke_rise))
            or not 0 < float(minimum_smoke_rise) <= MAX_SMOKE_RISE_PER_FRAME
        ):
            raise ValueError("minimum_smoke_rise exceeds the normalized bound")
        if (
        ...
        self._minimum_smoke_rise = float(minimum_smoke_rise)
    ...
    @property
    def metrics(self) -> FireSmokeReviewMetrics:
        return FireSmokeReviewMetrics(
    ...
    def confirm(self, frame: FireSmokeCandidateFrame) -> list[dict[str, object]]:
        ...
        if scope != self._scope:
            raise ValueError("fire/smoke candidate frame scope cannot change")
        if sequence <= self._last_sequence:
            raise ValueError("fire/smoke candidate sequence must increase")
        if self._last_observed is not None and observed_at <= self._last_observed:
            raise ValueError("fire/smoke candidate timestamps must strictly increase")
        ...
        for candidate_id, state in self._states.items():
            ...
            for index, candidate in enumerate(candidates):
                ...
                if (
                    ...
                    and _center_y(state.box) - _center_y(candidate.box)
                    < self._minimum_smoke_rise
                ):
                    continue
        ...
        for index, candidate in enumerate(candidates):
            ...
            previous = (
                self._states.get(candidate_id) if candidate_id is not None else None
            )
            if previous is None:
                if next_candidate_id > _MAX_SEQUENCE:
                    raise ValueError("fire/smoke candidate identifier space is exhausted")
            ...
            if consecutive >= _confirmation_frames(candidate.object_class) and not emitted:
                new_events.append(
                    _event(
                        self._scope,
                        candidate_id,
        ...
        if len(proposed) > self._max_candidates:
            raise ValueError("fire/smoke confirmation capacity is exhausted")
...
def _validate_frame(
    ...
    if not isinstance(frame, FireSmokeCandidateFrame):
        raise ValueError("frame must be a FireSmokeCandidateFrame")
    scope = tuple(
        _uuid(value, field)
        for value, field in (
...
def _validate_candidate(value: object) -> _CanonicalCandidate:
    if not isinstance(value, FireSmokeCandidate):
        raise ValueError("candidates must contain FireSmokeCandidate values")
    object_class = (
        value.object_class.strip().lower()
        if isinstance(value.object_class, str)
        else ""
    )
    if object_class not in FIRE_SMOKE_CLASSES:
        raise ValueError("fire/smoke candidate class is not permitted")
    if (
        ...
        raise ValueError("fire/smoke candidate confidence must be normalized")
    ...
    if any(
        ...
        raise ValueError("fire/smoke candidate box must contain finite numbers")
    box = tuple(float(item) for item in coordinates)
    if (
        not all(0 <= item <= 1 for item in box)
        or box[0] >= box[2]
        or box[1] >= box[3]
    ):
        raise ValueError("fire/smoke candidate box must be positive and normalized")
    if (
...
def _event(
    ...
    event_id = str(uuid5(_EVENT_NAMESPACE, f"{scope[0]}:{object_class}:{source}"))
    occurred_at = confirmed_at.isoformat(timespec="microseconds").replace(
        "+00:00", "Z"
    )
    event_type = f"{object_class}_review"
    return {
        "event_id": event_id,
        "tenant_id": scope[0],
...
def _confirmation_frames(object_class: str) -> int:
    return FIRE_CONFIRMATION_FRAMES if object_class == "fire" else SMOKE_CONFIRMATION_FRAMES


def _center_y(box: tuple[float, float, float, float]) -> float:
