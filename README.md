...
- `backend/`, `edge/`, `ai-services/`, and `frontend/` — composite services and language workspaces.
...
Phase 1 local persistence uses Postgres 16 with separate `syncam_admin` migration and `syncam_app` runtime roles. It includes tenant/site persistence, authenticated event ingestion, a leased transactional-outbox worker, and an idempotent alert queue projection. Copy `.env.example`, start `docker compose up -d postgres`, run `go run ./backend/cmd/migrate`, and follow the [Phase 1 development guide](docs/development/phase-1-identity.md). AWS is not required for this local workflow.
