...
| Cameras, device claims and heartbeat | T-0322–T-0329, T-0414 | `device`, HTTP handlers, edge runtime and libraries | Certificate/claim/heartbeat/RTSP/spool/runtime tests | Executable composes mTLS control loops and FFmpeg lifecycle; no compatible public mTLS endpoint or hardware evidence | PARTIAL | Add approved mTLS ingress, certify hardware, and connect decoded frames to inference |
| Zones, loitering and event-only vehicle/object logic | T-0307–T-0308, T-0330–T-0339, T-0416 | `zones`, `configdelivery`, post-mask frame sampler, Python track/zone/vehicle/abandoned-object modules | Deterministic unit and mask-before-sampling integration tests | Frame input is bounded after masking; no FFmpeg handoff or promoted detector/model artifacts | PARTIAL | Connect decode to the sampler, add approved inference, complete end-to-end integration and held-out evaluation |
| Privacy masks and evidence integrity | T-0340–T-0354, T-0415; FR-117 | `privacymasks`, edge release/HIL/hardware adapter and pre-analytics RGB24 renderer; `audit.Append` hash chain | Ledger/release/signature/hardware-boundary and synthetic pixel tests | Pixel renderer is implemented behind a matching activation; no FFmpeg wiring, physical result, or WORM deployment proved | PARTIAL | Wire the non-bypassable masked frame path, execute physical signed HIL, and add evidence object storage/access audit |
