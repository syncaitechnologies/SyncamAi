...
The T-0066 user-management foundation is currently an internal Go boundary only: `backend/internal/usermanagement` requires a verified, MFA-authenticated Super Admin with explicit `users:manage` scope before it can invoke a provider adapter. No HTTP route, browser write path, Supabase Admin adapter, user invitation, membership mutation, or bootstrap bypass is wired. A later implementation must first define an administrator bootstrap/recovery policy and use a tenant-scoped transaction to atomically write the requested membership change, audit event, and recoverable outbox message before provider delivery.
