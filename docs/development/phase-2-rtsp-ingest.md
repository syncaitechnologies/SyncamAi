...
T-0328 begins T-0154 with a bounded durable store-and-forward spool. Opaque event and evidence payloads are committed through a restricted temporary file, synced, and atomically renamed; startup verifies every committed record and removes interrupted temporary writes. Replay prioritizes metadata, then evidence, then archive video, preserving FIFO order within each class. Items remain on disk until explicit upstream acknowledgement, while quota pressure evicts the globally oldest committed item first. Queue depth, occupied bytes, oldest age, and enqueue, acknowledgement, and eviction counters are available for device telemetry. Payload ownership, transport retries, category-specific quotas, and hardware outage certification remain later integration work.
