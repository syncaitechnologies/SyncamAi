# Architecture decision records

Architecture decisions are immutable records: supersede an accepted ADR with a new ADR instead of rewriting its outcome. Proposed decisions may be edited until accepted.

| ADR | Decision | Status |
|---|---|---|
| [ADR-001](ADR-001-model-license.md) | Model and code licensing | Proposed; legal gate |
| [ADR-002](ADR-002-monorepo.md) | Monorepo | Accepted |
| [ADR-003](ADR-003-event-backbone.md) | Event backbone taxonomy | Accepted |
| [ADR-004](ADR-004-edge-agent-go.md) | Go edge agent | Accepted |
| [ADR-005](ADR-005-mvp-engine-scope.md) | Twelve-engine MVP | Accepted |
| [ADR-006](ADR-006-retention.md) | Retention range and presets | Accepted |
| [ADR-007](ADR-007-seed-roles.md) | Five seed roles | Accepted |
| [ADR-008](ADR-008-composite-deployment.md) | Composite MVP deployables | Accepted |
| [ADR-009](ADR-009-supabase-cloudflare-mvp-backend.md) | Temporary Supabase + Cloudflare MVP backend | Accepted |

Every ADR must state its operational and security consequences. ADR-001 remains a release blocker until legal approval is recorded.
